...
// HELPER MPYA KWA KUTENGENEZA SLUG SAFI (Consistent na HomePage)
...
export default function CategoryPage({ params }: { params?: { slug?: string } }) {
  ...
  // Format jina la category lionekane vizuri kwenye title
    ...
    const fetchData = async () => {
      try {
        ...
        if (prodRes.ok) {
          ...
          // Group and clean categories
          const rawCats = Array.from(new Set(data.map((p: any) => p.category))).filter(Boolean) as string[];

          // Map to standardized format and remove duplicates by slug
          const catMap = new Map();
          rawCats.forEach(c => {
            const slug = generateSlug(c);
            // Jina fupi kama "Computers & Laptops" inakuwa "Computers" kwa kioo tu (bado slug ni ileile)
            let display = c;
            if (c.toLowerCase().includes('computer')) display = 'Computers';
            if (c.toLowerCase().includes('phone')) display = 'Phones';
            if (c.toLowerCase().includes('fashion')) display = 'Fashion';

            if (!catMap.has(slug)) {
              catMap.set(slug, { name: display, slug: slug, original: c });
            }
          });

          const cleanedCats = Array.from(catMap.values());
          setAllCategories(cleanedCats);
        }
      } catch (error) {
  ...
  const ProductCard = ({ product }: { product: any }) => {
    ...
    if (viewMode === 'list') {
      return (
        <div onClick={() => router.push(`/product/${generateSlug(product.name)}`)} className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm flex gap-4 group hover:border-[#F2A900] transition cursor-pointer">
          <div className="relative w-32 h-32 bg-gray-50/50 rounded-xl flex items-center justify-center flex-shrink-0 p-2 overflow-hidden border border-gray-50">
            <span className="absolute top-2 left-2 bg-[#F2A900] text-black text-[10px] font-black px-1.5 py-0.5 rounded z-20">-{visualDiscount}%</span>
    ...
    return (
      <div onClick={() => router.push(`/product/${generateSlug(product.name)}`)} className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm flex flex-col h-full group hover:border-[#F2A900] transition cursor-pointer">
    ...
    <div className="min-h-screen bg-[#F8FAFC] font-sans text-gray-900">
      ...
      <main className="max-w-[1600px] mx-auto px-4 lg:px-6 py-6 flex gap-6 pb-20 md:pb-6">
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* Horizontal Sub-categories (MERGED AND CLEANED) */}
          <div className="flex overflow-x-auto hide-scrollbar gap-3 mb-8 pb-2">
            <div onClick={() => router.push('/categories')} className={`border-2 rounded-xl p-3 flex flex-col items-center justify-center min-w-[100px] cursor-pointer transition ${resolvedSlug === '' || resolvedSlug === 'all' ? 'bg-yellow-50 border-[#F2A900]' : 'bg-white border-gray-200'}`}>
            ...
            {allCategories.slice(0, 8).map((cat, idx) => (
              <div key={idx} onClick={() => router.push(`/categories/${cat.slug}`)} className={`border rounded-xl p-3 flex flex-col items-center justify-center min-w-[100px] cursor-pointer transition ${resolvedSlug === cat.slug ? 'bg-yellow-50 border-[#F2A900]' : 'bg-white border-gray-200 hover:border-[#F2A900]'}`}>
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="flex-1 w-full">
              ...
              {isLoading ? (
                ...
                <div className={`grid gap-3 sm:gap-4 w-full ${viewMode === 'grid' ? 'grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5' : 'grid-cols-1 md:grid-cols-2 lg:grid-cols-1'}`}>
                ...
                <div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl border border-gray-100 text-center w-full">
                  ...
                  <button onClick={resetFilters} className="mt-4 bg-[#0A101D] text-white px-6 py-2 rounded-lg text-sm font-bold hover:bg-[#F2A900] hover:text-black transition">Clear Filters</button>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 w-full bg-white border-t border-gray-100 flex justify-around items-center h-[60px] px-2 z-40 shadow-[0_-10px_20px_rgba(0,0,0,0.03)] pb-safe">
        ...
        <button onClick={() => router.push('/categories')} className="flex flex-col items-center gap-1 text-[#F2A900]"><FiGrid size={20} className="fill-current" /><span className="text-[9px] font-black">Categories</span></button>
