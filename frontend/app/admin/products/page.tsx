...
export default function AdminProducts() {
  ...
  const [specifications, setSpecifications] = useState(''); // Hii hapa
  
  // State kwa ajili ya faili la Picha
  ...
  const handleAddProduct = async (e: React.FormEvent) => {
    ...
    formData.append('specifications', specifications); // Tunatuma specifications
    ...
    try {
      ...
      setError('Tatizo la mtandao au server.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          ...
          <form onSubmit={handleAddProduct} className="space-y-4">
            ...
            {/* SEHEMU YA SPECIFICATIONS ILIYOKUWA IMESAHAULIKA */}
            <div>
                ...
                {products.map((p) => (
                  <tr key={p.id} className="border-b last:border-0 hover:bg-gray-50">
                    <td className="px-6 py-4 flex items-center gap-3">
                      {p.imageUrl ? (
                        <img src={`http://localhost:5001${p.imageUrl}`} alt={p.name} className="w-10 h-10 object-cover rounded-lg border border-gray-200" />
                      ) : (
